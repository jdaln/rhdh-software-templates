...
- [ ] 0- Prerequisites: Database Operator Installation
  - [ ] Verify EDB Postgres for Kubernetes Operator 1.27.1 is installed cluster-wide
    - [ ] Check operator status: `oc get ClusterServiceVersion/cloud-native-postgresql.v1.27.1`
    - [ ] Verify operator is in "Succeeded" phase
    - [ ] Ensure operator is available in target namespaces (or cluster-wide)
  - [ ] Verify storage classes are available for PostgreSQL persistent volumes
    - [ ] Check available storage classes: `oc get storageclass`
    - [ ] Default storage class should be configured
- [ ] 1- Project Provisioning per Team
  - [ ] Create namespaces for Lower Level Environment
    - [ ] Give users access to each LLE and registry
    - [ ] Projects setup
    - [ ] Resource Management
    - [ ] Ensure EDB operator can create resources in the namespace
- [ ] 2- Application Configuration in OpenShift
  ...
  - [ ] Database configuration (automatically handled by EDB operator)
    - [ ] PostgreSQL Cluster will be created automatically
...
- [ ] 4- Integrations required from OpenShift
  ...
  - [ ] Database monitoring (if required)
- [ ] 5- Security
  ...
  - [ ] Database credentials management (handled via Kubernetes Secrets)
